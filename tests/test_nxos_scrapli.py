"""Tests for NX-OS scrapli SSH transport.

Validates the NxosScrapliTransport class and its integration with
NxosDriver, including session reuse, failover from NX-API to SSH,
...
from network_mcp.drivers.cisco_nxos import HAS_SCRAPLI, NxosDriver, NxosScrapliTransport

# --- NxosScrapliTransport unit tests ---


class TestNxosScrapliTransportInit:
    """Test NxosScrapliTransport initialization."""

    @pytest.mark.skipif(not HAS_SCRAPLI, reason="scrapli not installed")
    def test_init_creates_driver(self):
        """Constructor creates a ScrapliNXOSDriver with correct params."""
        with patch("network_mcp.drivers.cisco_nxos.ScrapliNXOSDriver") as mock_cls:
            transport = NxosScrapliTransport(
                host="10.0.0.1",
                username="admin",
                password="cisco123",
                port=22,
...
class TestNxosScrapliTransportSendCommand:
    """Test NxosScrapliTransport.send_command()."""
    ...
    def test_send_command_returns_output(self):
        ...
        with patch("network_mcp.drivers.cisco_nxos.ScrapliNXOSDriver") as mock_cls:
            ...
            mock_response.result = "Cisco Nexus Operating System (NX-OS) Software"
            mock_driver.send_command.return_value = mock_response

            transport = NxosScrapliTransport(host="10.0.0.1", username="admin", password="cisco123")
            # Mark as connected to skip auto-open
            transport._connected = True

            output = transport.send_command("show version")
            assert output == "Cisco Nexus Operating System (NX-OS) Software"
...
class TestNxosScrapliTransportSendConfig:
    """Test NxosScrapliTransport.send_config()."""
...
class TestNxosScrapliTransportLifecycle:
...
# --- NxosDriver SSH integration tests ---


class TestNxosDriverScrapliIntegration:
    """Test NxosDriver SSH fallback using scrapli transport."""

    @pytest.mark.skipif(not HAS_SCRAPLI, reason="scrapli not installed")
    def test_nxapi_to_ssh_failover(self):
        """When NX-API fails, driver creates scrapli SSH transport."""
        driver = NxosDriver()

        with (
            patch("network_mcp.drivers.cisco_nxos.httpx") as mock_httpx,
            patch("network_mcp.drivers.cisco_nxos.ScrapliNXOSDriver") as mock_scrapli_cls,
        ):
    ...
    def test_ssh_direct_creates_scrapli_transport(self):
        ...
        with patch("network_mcp.drivers.cisco_nxos.ScrapliNXOSDriver") as mock_scrapli_cls:
            mock_scrapli_driver = MagicMock()
            mock_scrapli_cls.return_value = mock_scrapli_driver
    ...
    def test_ssh_session_reuse(self):
        ...
        with patch("network_mcp.drivers.cisco_nxos.ScrapliNXOSDriver") as mock_scrapli_cls:
            ...
            # ScrapliNXOSDriver was only instantiated once (session reuse)
    ...
    def test_close_cleans_up_scrapli(self):
        """close() cleans up both NX-API and scrapli transports."""
        driver = NxosDriver()
...
class TestNxosDriverNoScrapli:
    ...
    def test_ssh_fallback_without_scrapli_logs_warning(self):
        ...
        with (
            ...
            patch("network_mcp.drivers.cisco_nxos.logger") as mock_logger,
        ):
            driver.connect(host="10.0.0.1", username="admin", password="cisco123")
...
class TestNxosDriverSshKwargsPassthrough:
